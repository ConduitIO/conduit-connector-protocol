...
func TestSourceRunResponse(t *testing.T) {
	have := pconnector.SourceRunResponse{
		Records: []opencdc.Record{testRecordGo, testRecordGo},
	}
	want := []*connectorv1.Source_Run_Response{
	...
	is.Equal(
		"",
		cmp.Diff(want, got,
			cmpopts.IgnoreUnexported(connectorv1.Source_Run_Response{}),
...
func TestSourceStopResponse(t *testing.T) {
	have := pconnector.SourceStopResponse{
		LastPosition: []byte("test_position"),
	}
	want := &connectorv1.Source_Stop_Response{
		LastPosition: []byte("test_position"),
	}
	...
	is.Equal(
		"",
		cmp.Diff(want, got,
			cmpopts.IgnoreUnexported(connectorv1.Source_Stop_Response{}),
		),
	)
}
